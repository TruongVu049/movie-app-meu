...
import { Credit, Movie, TVDetail, Video } from "../types";
...
const TVDetailPage: React.FC = () => {
  ...
  const tvQuery = useMovies<TVDetail>({
    tags: [TAGS.tv, TAGS.detail, id],
    mediaType: "tv",
    categoryType: "detail",
    staleTime: 0,
    movieId: Number(id),
  ...
  let tv = null;
  if (!tvQuery.isLoading) tv = tvQuery.data as any;

  const castsQuery = useMovies<Credit>({
    tags: [TAGS.tv, TAGS.detail, TAGS.credit, id],
    mediaType: "tv",
    categoryType: "credit",
    staleTime: 0,
    movieId: Number(id),
  ...
  const videosQuery = useMovies<Video>({
    tags: [TAGS.tv, TAGS.detail, TAGS.video, id],
    mediaType: "tv",
    categoryType: "video",
    staleTime: 0,
    movieId: Number(id),
  ...
  const similarQuery = useMovies<Movie>({
    tags: [TAGS.tv, TAGS.detail, TAGS.similar, id],
    mediaType: "tv",
    ...
    <div>
      <div
        ...
        after:content-[''] after:absolute after:top-0 after:left-0 after:right-0 after:h-1/2 after:bg-gradient-to-t after:from-black ${
          tvQuery.isLoading ? "after:to-black/60" : "after:to-transparent"
        }  after:-z-10
          `}
      >
          ...
          <div className="px-4 flex-1 flex flex-col text-white">
            {tvQuery.isLoading ? (
              ...
              <>
                ...
                <p className="text-base md:py-4 py-2">{tv?.overview}</p>
              </>
            )}
